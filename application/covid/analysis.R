...
# Read data
df <- read.csv("aggregated_us_lev2_subset.csv")
...
form_ssn_mean <- ~ 
  ...
  s(temp) + s(humid) + 
  dnn(date, population, latitude, longitude, temp, humid)
form_nam_mean <- ~ 
...
ono <- deepregression(
  y = df$prevalence,
  list_of_formulas = list(form_ssn_mean, ~1),
  data = df,
...
if(file.exists("weights_ono.hdf5")){
  
  ono$model$load_weights(filepath="weights_ono.hdf5", by_name = FALSE)
  
}else{
  
  hist_ono <- ono %>% fit(epochs = 100L, early_stopping = TRUE, 
                          patience = 3L, batch_size = 1024L)
  
  save_model_weights_hdf5(ono$model, filepath="weights_ono.hdf5")
  
}


Metrics::rmse(test$prevalence, ono %>% predict(test))
# 0.6670314

unconstrained <- deepregression(
  y = df$prevalence,
  list_of_formulas = list(form_ssn_mean, ~1),
  data = df,
  list_of_deep_models = list(
...
if(file.exists("weights_unc.hdf5")){
  
  unconstrained$model$load_weights(filepath="weights_unc.hdf5", by_name = FALSE)
  
}else{
  
  hist_unc <- unconstrained %>% fit(epochs = 100L, early_stopping = TRUE, 
                                    patience = 3L, batch_size = 1024L)
  
  save_model_weights_hdf5(unconstrained$model, filepath="weights_unc.hdf5")
  
}

Metrics::rmse(test$prevalence, unconstrained %>% predict(test))
# 0.7142373

nam <- deepregression(
  y = df$prevalence,
  list_of_formulas = list(form_nam_mean, ~1),
  data = df,
  list_of_deep_models = list(
    dnn = deep_mod_nam,
    snam = feature_net,
...
if(file.exists("weights_nam.hdf5")){
  ...
  nam$model$load_weights(filepath="weights_nam.hdf5", by_name = FALSE)
  
}else{
  
  hist_nam <- nam %>% fit(epochs = 150L, early_stopping = TRUE, 
                          patience = 15L, batch_size = 1024L)
  
  save_model_weights_hdf5(nam$model, filepath="weights_nam.hdf5")
   
}

Metrics::rmse(test$prevalence, nam %>% predict(test))
# 0.6670314
